...
public class Hands : MonoBehaviour{
    ...
    void Update() {
        basicMovement(leftSteer, leftHand);
		basicMovement(rightSteer, rightHand);
		shiftingMovement();
		leftSteer = holdWheel(leftHand, leftPalm, leftFist, leftSteer);
		rightSteer = holdWheel(rightHand, rightPalm, rightFist, rightSteer);
		leftSteer = letGoWheel(leftHand, leftPalm, leftFist, leftSteer);
		rightSteer = letGoWheel(rightHand, rightPalm, rightFist, rightSteer);
		stopPointing();
		initSignalling();
		signalDirection();
		stopSignalling();
	...
	private bool letGoWheel(GameObject hand, GameObject palm, GameObject fist, bool steer) {
		if (Input.GetKeyDown("space") && (leftSteer || rightSteer)) {
	...
	private void shiftingMovement() {
		...
		if (GetComponent<GearShift>().shifting) {
			...
			if (rightHand.transform.position[2] < top || v < 0 ) {
				if (rightHand.transform.position[2] > bottom || v > 0) {
					rightHand.transform.Translate(0,0,v);
				}
			}
		}
	}
	...
	private void basicMovement(bool steer, GameObject hand) {
		float h = Input.GetAxis("Horizontal");
		float v = Input.GetAxis("Vertical");
		if (!steer && !signalling) {
			hand.transform.Translate(h,0,v);
